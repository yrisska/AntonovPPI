...
namespace WebApplication1.Services.BookService
{
    public class BookService : IBookService
    {
        ...
        private void SeedBooks()
        {
            ...
            var task = _publisherService.GetPublishers();
            task.Wait();
            ...
            var bigData = new Book()
            {
                ...
                Publisher = publishers.FirstOrDefault(publisher => publisher.Name == "Manning Publications"),
                Price = 120
            };
            ...
            var adaptiveCode = new Book()
            {
                ...
                Publisher = publishers.FirstOrDefault(publisher => publisher.Name == "Microsoft Press"),
                Price = 50
            };
            ...
            var sqlPerformanceTuning = new Book()
            {
                ...
                Publisher = publishers.FirstOrDefault(publisher => publisher.Name == "Apress"),
                Price = 40
            };
            ...
            var buildingMicroservices = new Book()
            {
                ...
                Publisher = publishers.FirstOrDefault(publisher => publisher.Name == "Packt Publishing"),
                Price = 32
            };
            ...
            var concurrency = new Book()
            {
                ...
                Publisher = publishers.FirstOrDefault(publisher => publisher.Name == "O'Reilly Media"),
                Price = 45
            };

            var machineLearning = new Book()
            {
                Id = Guid.NewGuid(),
                Title = "Mastering Machine Learning with Python",
                Description = "Mastering Machine Learning with Python is a comprehensive guide that explores the concepts and techniques of machine learning using the Python programming language. This book covers a wide range of topics, including supervised and unsupervised learning, deep learning, and reinforcement learning. It provides practical examples and real-world projects to help you apply machine learning algorithms effectively.",
                PublishedOn = new DateTime(2022, 1, 15),
                Publisher = publishers.FirstOrDefault(publisher => publisher.Name == "Packt Publishing"),
                Price = 55
            };

            var dataScience = new Book()
            {
                Id = Guid.NewGuid(),
                Title = "Data Science for Beginners",
                Description = "Data Science for Beginners is an introductory book that provides an overview of the field of data science. It covers the fundamental concepts, techniques, and tools used in data science, such as data analysis, data visualization, and machine learning. This book is designed for readers with little to no background in data science and aims to provide a solid foundation for further learning.",
                PublishedOn = new DateTime(2023, 5, 5),
                Publisher = publishers.FirstOrDefault(publisher => publisher.Name == "O'Reilly Media"),
                Price = 30
            };

            var dataVizualization = new Book()
            {
                Id = Guid.NewGuid(),
                Title = "The Art of Data Visualization",
                Description = "The Art of Data Visualization is a visually stunning book that explores the principles and techniques of creating effective and captivating data visualizations. It covers the importance of storytelling through data, the use of color and layout, and the selection of appropriate visualization tools. This book is a valuable resource for data analysts, designers, and anyone interested in presenting data in a compelling and informative way.",
                PublishedOn = new DateTime(2022, 11, 12),
                Publisher = publishers.FirstOrDefault(publisher => publisher.Name == "Manning Publications"),
                Price = 30
            };

            _bookRepository.Add(bigData);
            _bookRepository.Add(kubernetes);
            _bookRepository.Add(adaptiveCode);
            _bookRepository.Add(cSharpInDepth);
            _bookRepository.Add(sqlPerformanceTuning);
            _bookRepository.Add(buildingMicroservices);
            _bookRepository.Add(concurrency);
            _bookRepository.Add(machineLearning);
            _bookRepository.Add(dataScience);
            _bookRepository.Add(dataVizualization);

        }

        public async Task<Book> CreateBook(BookForUpdate bookForUpdate)
        {
        ...
        {
            var book = _bookRepository.FirstOrDefault(x => x.Id == id) ?? throw new KeyNotFoundException("No book with such id");
        ...
        {
            return await Task.FromResult(_bookRepository);
        }

        public async Task UpdateBook(Guid id, BookForUpdate bookForUpdate)
        {
            var book = _bookRepository.FirstOrDefault(x => x.Id == id) ?? throw new KeyNotFoundException("No book with such id");

            book.Title = bookForUpdate.Title;
            book.Description = bookForUpdate.Description;
            book.Price = bookForUpdate.Price;
            book.PublishedOn = bookForUpdate.PublishedOn;
            book.Publisher = await _publisherService.GetPublisher(bookForUpdate.PublisherId);
